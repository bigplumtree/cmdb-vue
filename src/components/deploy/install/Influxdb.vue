<template>
  <div>
    <!-- 头部导航区 -->
    <el-page-header @back="goBack" content="Influxdb"></el-page-header>
    ...
    <!-- 进度条
      ...
      <el-tabs v-model="activeIndex" :before-leave="beforeTabLeave"  :stretch="true">
        ...
        <el-tab-pane label="参数" name="1">
          <el-form :model="softInstallParamters" :rules="softInstallFormRules" ref="installStartFormRef" label-width="180px">
            ...
            <el-row>
              <el-col :span="10">
                <el-form-item label="超级管理员密码" prop="influxdb_password">
                  <el-input width="80px" placeholder="大小写字母,数字,字符最少包含两种,不少于9位" v-model="softInstallParamters.influxdb_password"></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="10">
                <el-form-item label="influxdb工作目录"  prop="influxdb_workpath">
                  <el-input width="80px" placeholder="例如: /data" v-model="softInstallParamters.influxdb_workpath"></el-input>
                </el-form-item>
              </el-col>
            </el-row>
            ...
            <el-row>
              <el-col :span="10">
                <el-form-item label="influxdb启动端口" prop="influxdb_port" >
                  <el-input width="80px" placeholder="例如:18086" v-model.number="softInstallParamters.influxdb_port"></el-input>
                </el-form-item>
              </el-col>
            </el-row>
      ...
      <el-col :span="6" >
        <el-card class="infoCard">
          ...
          <el-alert class="rightAlert" title="Influxdb其他配置项目前先不提供修改" type="info" :closable="false" show-icon></el-alert>
          <el-alert class="rightAlert" title="当前安装的版本是Influxdb-1.8.0,暂不提供其他版本" type="info" :closable="false" show-icon></el-alert>                
        </el-card>
      </el-col>
    </el-row> 
...
export default {
    ...
    data() {
      ...
      var password_check = (rule, value, callback) => {
        var password_reg = /(?=.*\d)(?=.*[a-zA-Z])(?=.*[^a-zA-Z0-9]).{9,16}/
        if (!value) {
          return callback(new Error('密码不能为空'));
        }else if(value.indexOf(' ') !== -1){
          return callback(new Error('密码中不能包含空格'));
        }else if(!password_reg.test(value)){
          return callback(new Error('密码必须由数字、大小写字母、特殊字符三种组合,请输入9-16位'))
        }else{
          callback()
        }
      };
      ...
      return {
        ...
        softInstallParamters: {
          close_selinux: true,
          close_firewall: true ,
          influxdb_password: 'qwe-ASD1!',
          influxdb_port: 8086,
          influxdb_workpath: '/data',        
        },
        ...
        softInstallFormRules: {
          ...
          influxdb_port: [
            { required: true, message: '请输入启动端口', trigger: 'blur'},
        ...
        this.$refs.installStartFormRef.validate((valid, obj) => {
          ...
          if (!valid) 
            ...
            let params = new URLSearchParams();
              ...
              params.append('influxdb_password', this.softInstallParamters.influxdb_password)
              params.append('influxdb_workpath', this.softInstallParamters.influxdb_workpath)
              params.append('influxdb_port', this.softInstallParamters.influxdb_port)
              this.$axios
                .post('deploy/influxdb/install/', params, {timeout: 0})
                .then(success => {
                  if(success.data.status === 200){

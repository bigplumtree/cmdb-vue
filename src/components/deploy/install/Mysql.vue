<template>
  <div>
    <!-- 头部导航区 -->
    <el-page-header @back="goBack" content="Mysql"></el-page-header>
    ...
    <!-- 进度条
      ...
      <el-tabs v-model="activeIndex" :before-leave="beforeTabLeave"  :stretch="true">
        ...
        <el-tab-pane label="参数" name="1">
          <el-form :model="softInstallParamters" :rules="softInstallFormRules" ref="installStartFormRef" label-width="180px">
            ...
            <el-row>
              <el-col :span="10">
                <el-form-item label="root密码" prop="mysql_root_password">
                  <el-input width="80px" placeholder="大小写字母,数字,字符最少包含三种,不少于9位" v-model="softInstallParamters.mysql_root_password"></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="10">
                <el-form-item label="mysql工作目录"  prop="mysql_work_dir">
                  <el-input width="80px" placeholder="例如: /data" v-model="softInstallParamters.mysql_work_dir"></el-input>
                </el-form-item>
              </el-col>
            </el-row>
            ...
            <el-row>
              <el-col :span="10">
                <el-form-item label="innodb缓存池大小(GB)" prop="innodb_buffer_pool_size" >
                  <el-input width="80px" placeholder="例如:2" v-model.number="softInstallParamters.innodb_buffer_pool_size"></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="10">
                <el-form-item label="mysql启动端口" prop="mysql_port" >
                  <el-input width="80px" placeholder="例如:13303" v-model.number="softInstallParamters.mysql_port"></el-input>
                </el-form-item>
              </el-col>
            </el-row>
      ...
      <el-col :span="6" >
        <el-card class="infoCard">
          ...
          <el-alert class="rightAlert" title="innodb缓存池大小建议为物理内存的一半,单位GB" type="info" :closable="false" show-icon></el-alert>
          <el-alert class="rightAlert" title="MySQL其他配置项目前先不提供修改" type="info" :closable="false" show-icon></el-alert>
          <el-alert class="rightAlert" title="安装过程会传输一个500MB大小的安装包,100M网络带宽情况下,整个部署时间大约需要5分钟" type="warning" :closable="false" show-icon></el-alert>          
        </el-card>
      </el-col>
    </el-row> 
      ...
      return {
        ...
        softInstallFormRules: {
          ...
          innodb_buffer_pool_size: [
            ...
            { type: 'number', message: '大小必须为数值', trigger: 'blur'}
          ], 
          mysql_port: [
        ...
        this.$refs.installStartFormRef.validate((valid, obj) => {
          ...
          if (!valid) 
            ...
            let params = new URLSearchParams();
              ...
              params.append('mysql_root_password', this.softInstallParamters.mysql_root_password)
              params.append('innodb_buffer_pool_size', this.softInstallParamters.innodb_buffer_pool_size)
              params.append('mysql_port', this.softInstallParamters.mysql_port)
              params.append('mysql_work_dir', this.softInstallParamters.mysql_work_dir)
              this.$axios
                .post('deploy/mysql/install/', params, {timeout: 0})
                .then(success => {
                  if(success.data.status === 200){

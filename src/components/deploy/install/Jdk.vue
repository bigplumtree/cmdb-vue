<template>
  <div>
    <!-- 头部导航区 -->
    <el-page-header @back="goBack" content="JDK安装"></el-page-header>
    ...
    <!-- 进度条
      ...
      <el-col :span="6" >
        <el-card class="infoCard">
          <el-alert class="rightAlert" title="SElinux和firewall建议关闭,也可以自行选择保持原有状态(如果不关闭,后期可能会出现一些问题)" type="warning" :closable="false" show-icon></el-alert>
          <el-alert class="rightAlert" title="jdk默认安装到/usr/local/java目录下,目前阶段不提供修改路径" type="warning" :closable="false" show-icon></el-alert>
          <el-alert class="rightAlert" title="当前安装的jdk版本为1.8.0_144,暂不提供其他版本" type="warning" :closable="false" show-icon></el-alert>
        </el-card>
      </el-col>
    </el-row> 
    ...
    methods: {
      ...
      install_start(){
        ...
        let params = new URLSearchParams();
          ...
          this.$axios
            .post('deploy/jdk/install/', params, {timeout: 0})
            .then(success => {
              if(success.data.status === 200){
